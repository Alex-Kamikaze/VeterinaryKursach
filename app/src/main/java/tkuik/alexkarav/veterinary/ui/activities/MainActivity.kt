...
import tkuik.alexkarav.veterinary.ui.components.main_page.MainPage
                    ...
                    NavHost(navController = navController, startDestination = if(introScreenCompleted == true) "auth" else "intro_screen") {
                            ...
                            Column(
                                ...
                                val screens = listOf(
                                    IntroScreenModel(
                                        ...
                                        "Профессиональные ветеринары с огромным опытом работы с животными",
                                        R.drawable.intro_screen_image_1
                                    ),
                                    IntroScreenModel(
                                        "Удобство и доступность",
                                        "Быстрые анализы и доступ к результатам через мобильное приложение",
                                        R.drawable.intro_screen_image_2
                                    ),
                                    IntroScreenModel(
                                        "Высокие технологии",
                                        "Современные технологии в медицине для помощи вашему питомцу",
                                        R.drawable.intro_screen_image_3
                                    )
                                )
                                IntroScreen(
                                    screens = screens, modifier = Modifier
                                        .fillMaxSize()
                                        .padding(40.dp)
                                ) {
                        ...
                        composable("auth") {
                            ...
                            if (authorizationState.value?.isSuccess == true) {
                                ...
                                Toast.makeText(
                                    ...
                                    authorizationState.value?.exceptionOrNull().toString(),
                                    Toast.LENGTH_SHORT
                                ).show()
                            ...
                            AuthScreen { event ->
                                when (event) {
                                    ...
                                    AuthScreenUIEvents.OnRegisterButtonPressed -> navController.navigate(
                                        "registration"
                                    )
                                }
                            }
                        ...
                        composable("registration") {
                            ...
                            val registrationState =
                                registerViewModel.registrationState.collectAsState()
...
//                                          registerViewModel.onRegistrationComplete()
                                    },
                                ...
                                Toast.makeText(
                                    LocalContext.current,
                                    registrationState.value?.exceptionOrNull().toString(),
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
                            RegistrationScreen { event ->
                                when (event) {
                                    is RegistrationScreenUIEvents.OnRegisterButtonPressed -> registerViewModel.onEvent(
                                        event
                                    )
                                }
                            }
